#include "subghz_wardiving_history.h"
...
SubGhzHistory* subghz_wardiving_history_alloc(void) {
...
void subghz_wardiving_history_free(SubGhzHistory* instance) {
...
uint32_t subghz_wardiving_history_get_hash_data(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
...
const SubGhzProtocol*
    subghz_wardiving_history_get_protocol(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
    return item->protocol;
}

uint16_t subghz_wardiving_history_get_repeats(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
    return item->repeats;
}

uint32_t subghz_wardiving_history_get_frequency(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
...
SubGhzRadioPreset*
    subghz_wardiving_history_get_radio_preset(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
    return item->preset;
}

const char* subghz_wardiving_history_get_preset(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
    return furi_string_get_cstr(item->preset->name);
}

float subghz_wardiving_history_get_latitude(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
    return item->latitude;
}

float subghz_wardiving_history_get_longitude(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
    return item->longitude;
}

void subghz_wardiving_history_reset(SubGhzHistory* instance) {
    furi_assert(instance);
...
void subghz_wardiving_history_delete_item(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
...
uint16_t subghz_wardiving_history_get_item(SubGhzHistory* instance) {
    furi_assert(instance);
    return instance->last_index_write;
}

uint8_t subghz_wardiving_history_get_type_protocol(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
    return item->type;
}

const char* subghz_wardiving_history_get_protocol_name(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
...
DateTime subghz_wardiving_history_get_datetime(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
    if(item) {
...
FlipperFormat* subghz_wardiving_history_get_raw_data(SubGhzHistory* instance, uint16_t idx) {
    furi_assert(instance);
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
...
bool subghz_wardiving_history_get_text_space_left(
...
uint16_t subghz_wardiving_history_get_last_index(SubGhzHistory* instance) {
    return instance->last_index_write;
}
void subghz_wardiving_history_get_text_item_menu(
    SubGhzHistory* instance,
    FuriString* output,
    uint16_t idx) {
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
...
void subghz_wardiving_history_get_time_item_menu(
    SubGhzHistory* instance,
    FuriString* output,
    uint16_t idx) {
    SubGhzHistoryItem* item = SubGhzHistoryItemArray_get(instance->history->data, idx);
...
bool subghz_wardiving_history_add_to_history(
    SubGhzHistory* instance,
    void* context,
    SubGhzRadioPreset* preset,
    float latitude,
...
void subghz_wardiving_history_remove_duplicates(SubGhzHistory* instance) {
    furi_assert(instance);
    ...
    while(!SubGhzHistoryItemArray_end_p(it)) {
        ...
        while(!SubGhzHistoryItemArray_end_p(jt)) {
            ...
            if(j->hash_data == i->hash_data && j->protocol == i->protocol) {
                subghz_wardiving_history_delete_item(instance, jt->index);
            }
            SubGhzHistoryItemArray_previous(jt);
        }
        SubGhzHistoryItemArray_previous(it);
...
bool subghz_wardiving_history_full(SubGhzHistory* instance) {
