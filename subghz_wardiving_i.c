...
bool subghz_tx_start(SubGhz* subghz, FlipperFormat* flipper_format) {
    switch(subghz_wardiving_txrx_tx_start(subghz->txrx, flipper_format)) {
...
bool subghz_key_load(SubGhz* subghz, const char* file_path, bool show_dialog) {
    ...
    Stream* fff_data_stream =
        flipper_format_get_raw_stream(subghz_wardiving_txrx_get_fff_data(subghz->txrx));
    ...
    do {
        ...
        if(!subghz_wardiving_txrx_radio_device_is_frequency_valid(subghz->txrx, temp_data32)) {
        ...
        furi_string_set_str(
            temp_str,
            subghz_wardiving_txrx_get_preset_name(subghz->txrx, furi_string_get_cstr(temp_str)));
        ...
        if(!strcmp(furi_string_get_cstr(temp_str), "RAW")) {
            ...
            subghz_protocol_raw_gen_fff_data(
                fff_data, file_path, subghz_wardiving_txrx_radio_device_get_name(subghz->txrx));
        } else {
        ...
        if(subghz_wardiving_txrx_load_decoder_by_name_protocol(
            ...
            SubGhzProtocolStatus status = subghz_protocol_decoder_base_deserialize(
                subghz_wardiving_txrx_get_decoder(subghz->txrx), fff_data);
