...
SubGhz* subghz_alloc(bool alloc_for_tx_only) {
...
#endif
    subghz->txrx = subghz_wardiving_txrx_alloc();
    ...
    if(!alloc_for_tx_only) {
        ...
        subghz_wardiving_txrx_set_preset_internal(
            ...
            subghz->tx_power);
        subghz->history = subghz_wardiving_history_alloc();
    }
    ...
    subghz_wardiving_txrx_set_need_save_callback(subghz->txrx, subghz_save_to_file, subghz);
...
void subghz_free(SubGhz* subghz, bool alloc_for_tx_only) {
    ...
    subghz_wardiving_txrx_speaker_off(subghz->txrx);
    subghz_wardiving_txrx_stop(subghz->txrx);
    subghz_wardiving_txrx_sleep(subghz->txrx);
...
int32_t subghz_app(char* p) {
    ...
    if(subghz_wardiving_txrx_is_database_loaded(subghz->txrx)) {
