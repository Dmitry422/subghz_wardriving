...
uint8_t subghz_scene_receiver_config_next_frequency(const uint32_t value, void* context) {
    ...
    SubGhzSetting* setting = subghz_wardiving_txrx_get_setting(subghz->txrx);
...
static void subghz_scene_receiver_config_set_frequency(VariableItem* item) {
    ...
    if(subghz_wardiving_txrx_hopper_get_state(subghz->txrx) == SubGhzHopperStateOFF) {
        ...
        //Set TX Power
        subghz_wardiving_txrx_set_tx_power(preset.data, preset.data_size, subghz->tx_power);
        ...
        preset = subghz_wardiving_txrx_get_preset(subghz->txrx);
...
static void subghz_scene_receiver_config_set_preset(VariableItem* item) {
    ...
    subghz_wardiving_txrx_set_tx_power(preset_data, preset_data_size, subghz->tx_power);
...
static void subghz_scene_receiver_config_set_hopping(VariableItem* item) {
    ...
    subghz_wardiving_txrx_hopper_set_state(
...
static void subghz_scene_receiver_config_set_speaker(VariableItem* item) {
    ...
    variable_item_set_current_value_text(item, combobox_text[index]);
    subghz_wardiving_txrx_speaker_set_state(subghz->txrx, speaker_value[index]);
...
static void subghz_scene_receiver_config_set_repeater(VariableItem* item) {
    ...
    if(subghz->repeater == SubGhzRepeaterStateOff &&
       subghz_wardiving_history_get_last_index(subghz->history)) {
...
static void subghz_scene_receiver_config_set_duplicates(VariableItem* item) {
    ...
    if(index) subghz_wardiving_history_remove_duplicates(subghz->history);
}
...
static void subghz_scene_receiver_config_var_list_enter_callback(void* context, uint32_t index) {
    ...
    if(index == SubGhzSettingIndexRemoveDuplicates) {
        ...
        subghz_wardiving_txrx_speaker_set_state(subghz->txrx, speaker_value[default_index]);
        subghz->last_settings->enable_sound = false;

        subghz_wardiving_txrx_hopper_set_state(subghz->txrx, hopping_value[default_index]);
...
void subghz_scene_receiver_config_on_enter(void* context) {
    ...
    value_index = value_index_uint32(
        subghz_wardiving_txrx_speaker_get_state(subghz->txrx), speaker_value, COMBO_BOX_COUNT);
    variable_item_set_current_value_index(item, value_index);
    variable_item_set_current_value_text(item, combobox_text[value_index]);
...
bool subghz_scene_receiver_config_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == SubGhzCustomEventSceneSettingRepeater) {
            ...
            subghz_wardiving_history_remove_duplicates(subghz->history);
