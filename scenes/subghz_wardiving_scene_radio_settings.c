...
static void subghz_scene_radio_settings_set_device(VariableItem* item) {
    ...
    if(!subghz_wardiving_txrx_radio_device_is_external_connected(
    ...
    subghz_wardiving_txrx_radio_device_set(subghz->txrx, radio_device_value[index]);
}
...
static void subghz_scene_receiver_config_set_debug_pin(VariableItem* item) {
    ...
    subghz_wardiving_txrx_set_debug_pin_state(subghz->txrx, index == 1);
}
...
void subghz_scene_radio_settings_on_enter(void* context) {
    ...
    if(subghz_wardiving_txrx_radio_device_get(subghz->txrx) == SubGhzRadioDeviceTypeInternal &&
    ...
    item = variable_item_list_add(
        ...
        subghz);
    value_index = subghz_wardiving_txrx_get_debug_pin_state(subghz->txrx);
    variable_item_set_current_value_index(item, value_index);
