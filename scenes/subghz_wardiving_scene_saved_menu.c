#include "../subghz_wardiving_i.h" // IWYU pragma: keep
...
void subghz_scene_saved_menu_on_enter(void* context) {
    ...
    if(!isnanf(subghz_wardiving_txrx_get_latitude(subghz->txrx)) ||
       !isnanf(subghz_wardiving_txrx_get_longitude(subghz->txrx)) || subghz->gps) {
        submenu_add_item(
            subghz->submenu,
            "Geographic info",
            SubmenuIndexGeo,
