...
bool subghz_scene_transmitter_update_data_show(void* context) {
    SubGhz* subghz = context;
    bool ret = false;
    SubGhzProtocolDecoderBase* decoder = subghz_wardiving_txrx_get_decoder(subghz->txrx);
    ...
    if(decoder) {
        ...
        if(subghz_protocol_decoder_base_deserialize(
               decoder, subghz_wardiving_txrx_get_fff_data(subghz->txrx)) ==
           SubGhzProtocolStatusOk) {
            ...
            subghz_view_transmitter_add_data_to_show(
                ...
                subghz_wardiving_txrx_protocol_is_transmittable(subghz->txrx, false));
    ...
    subghz_view_transmitter_set_radio_device_type(
        subghz->subghz_transmitter, subghz_wardiving_txrx_radio_device_get(subghz->txrx));
    return ret;
}
        ...
        if(subghz->state_notifications == SubGhzNotificationStateTx) {
            ...
            if(!subghz_devices_is_async_complete_tx(subghz->txrx->radio_device)) {
                ...
                if(subghz_custom_btn_get() != SUBGHZ_CUSTOM_BTN_OK) {
                    ...
                    // Calling restore!
                    subghz_tx_start(subghz, subghz_wardiving_txrx_get_fff_data(subghz->txrx));
                    subghz_wardiving_txrx_stop(subghz->txrx);
