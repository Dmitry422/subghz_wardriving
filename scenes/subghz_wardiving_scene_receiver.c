...
static void subghz_scene_receiver_update_statusbar(void* context) {
    ...
    if(!subghz_wardiving_history_get_text_space_left(
           subghz->history,
           history_stat_str,
        ...
        if(subghz_wardiving_history_get_last_index(subghz->history) > 0) {
            subghz_wardiving_txrx_get_frequency_and_modulation(
            ...
            furi_string_printf(
                modulation_str,
                "%s        Mod: %s",
                (subghz_wardiving_txrx_radio_device_get(subghz->txrx) ==
        ...
        subghz_view_receiver_add_data_statusbar(
            ...
            subghz_wardiving_txrx_hopper_get_state(subghz->txrx) != SubGhzHopperStateOFF,
    ...
    subghz_view_receiver_set_radio_device_type(
        subghz->subghz_receiver, subghz_wardiving_txrx_radio_device_get(subghz->txrx));
}
...
static void subghz_scene_add_to_history_callback(
    ...
    uint16_t idx = subghz_wardiving_history_get_item(history);

    SubGhzRadioPreset preset = subghz_wardiving_txrx_get_preset(subghz->txrx);
    ...
    if(subghz->last_settings->delete_old_signals &&
       subghz_wardiving_history_full(subghz->history)) {
        subghz_view_receiver_disable_draw_callback(subghz->subghz_receiver);

        while(idx > 0 && subghz_wardiving_history_full(subghz->history)) {
            subghz_wardiving_history_delete_item(subghz->history, 0);
    ...
    if(subghz_wardiving_history_add_to_history(
        ...
        if(subghz->repeater != SubGhzRepeaterStateOff) {
            ...
            if(subghz->remove_duplicates) {
                ...
                for(uint16_t i = idx; i > 0; i--) {
                    ...
                    if(subghz_wardiving_history_get_hash_data(subghz->history, i) == hash_data &&
                       subghz_wardiving_history_get_protocol(subghz->history, i) ==
                           decoder_base->protocol) {
                        // Remove previous instance and update menu index
                        subghz_wardiving_history_delete_item(subghz->history, i);
            ...
            subghz_wardiving_history_get_text_item_menu(history, item_name, idx);
            subghz_wardiving_history_get_time_item_menu(history, item_time, idx);
            subghz_view_receiver_add_item_to_menu(
                subghz->subghz_receiver,
                furi_string_get_cstr(item_name),
                furi_string_get_cstr(item_time),
                subghz_wardiving_history_get_type_protocol(history, idx),
                subghz_wardiving_history_get_repeats(history, idx));
            ...
            if(decoder_base->protocol->flag & SubGhzProtocolFlag_Save &&
               subghz->last_settings->autosave) {
                ...
                DateTime time = subghz_wardiving_history_get_datetime(history, idx);
                ...
                subghz_save_protocol_to_file(
                    subghz,
                    subghz_wardiving_history_get_raw_data(history, idx),
...
void subghz_scene_receiver_on_enter(void* context) {
    ...
    if(subghz_rx_key_state_get(subghz) == SubGhzRxKeyStateIDLE) {
        subghz_wardiving_txrx_set_preset_internal(
            subghz->txrx,
        ...
        subghz_wardiving_txrx_receiver_set_filter(subghz->txrx, subghz->filter);
        subghz->ignore_filter = subghz->last_settings->ignore_filter;
        //subghz_wardiving_txrx_receiver_set_ignore_filter(subghz->txrx, subghz->ignore_filter);
        subghz->tx_power = subghz->last_settings->tx_power;

        subghz_wardiving_history_reset(history);
        subghz_rx_key_state_set(subghz, SubGhzRxKeyStateStart);
    ...
    for(uint16_t i = 0; i < subghz_wardiving_history_get_item(history); i++) {
        furi_string_reset(item_name);
        furi_string_reset(item_time);
        subghz_wardiving_history_get_text_item_menu(history, item_name, i);
        subghz_wardiving_history_get_time_item_menu(history, item_time, i);
        subghz_view_receiver_add_item_to_menu(
            subghz->subghz_receiver,
            furi_string_get_cstr(item_name),
            furi_string_get_cstr(item_time),
            subghz_wardiving_history_get_type_protocol(history, i),
            subghz_wardiving_history_get_repeats(history, i));
        subghz_rx_key_state_set(subghz, SubGhzRxKeyStateAddKey);
    }
    furi_string_free(item_name);
    furi_string_free(item_time);
    ...
    subghz_wardiving_txrx_set_rx_callback(
    ...
    if(subghz->last_settings->enable_hopping) {
        subghz_wardiving_txrx_hopper_set_state(subghz->txrx, SubGhzHopperStateRunning);
    } else {
        subghz_wardiving_txrx_hopper_set_state(subghz->txrx, SubGhzHopperStateOFF);
    }

    // Check if Sound was enabled, and restart the Speaker.
    subghz_wardiving_txrx_speaker_set_state(
        subghz->txrx,
    ...
    furi_check(subghz_wardiving_txrx_load_decoder_by_name_protocol(
...
bool subghz_scene_receiver_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        case SubGhzCustomEventViewReceiverBack:
            // Stop CC1101 Rx
            subghz->state_notifications = SubGhzNotificationStateIDLE;
            subghz_wardiving_txrx_stop(subghz->txrx);
            subghz_wardiving_txrx_hopper_set_state(subghz->txrx, SubGhzHopperStateOFF);
            subghz_wardiving_txrx_set_rx_callback(subghz->txrx, NULL, subghz);

            if(subghz_wardiving_history_get_last_index(subghz->history)) {
                subghz_rx_key_state_set(subghz, SubGhzRxKeyStateExit);
                scene_manager_next_scene(subghz->scene_manager, SubGhzSceneNeedSaving);
            } else {
                subghz_rx_key_state_set(subghz, SubGhzRxKeyStateIDLE);
                subghz_wardiving_txrx_set_default_preset(
        ...
        case SubGhzCustomEventViewReceiverDeleteItem:
            ...
            subghz_wardiving_history_delete_item(subghz->history, subghz->idx_menu_chosen);
        ...
        case SubGhzCustomEventViewRepeaterStart:
            subghz_wardiving_txrx_stop(subghz->txrx);
            subghz_wardiving_txrx_hopper_pause(subghz->txrx);

            FlipperFormat* key_repeat_data = subghz_wardiving_history_get_raw_data(
                subghz->history, subghz_wardiving_history_get_last_index(subghz->history) - 1);
            ...
            if(subghz_wardiving_txrx_tx_start(subghz->txrx, key_repeat_data) !=
        ...
        case SubGhzCustomEventViewRepeaterStop:
            subghz_wardiving_txrx_stop(subghz->txrx);
            subghz_wardiving_history_delete_item(
                subghz->history, subghz_wardiving_history_get_last_index(subghz->history) - 1);
            subghz_wardiving_txrx_rx_start(subghz->txrx);
        ...
        case SubGhzCustomEventViewReceiverOKLong:
            subghz_wardiving_txrx_stop(subghz->txrx);
            subghz_wardiving_txrx_hopper_pause(subghz->txrx);
            if(subghz_wardiving_txrx_tx_start(
                   subghz->txrx,
        ...
        if(subghz_rx_key_state_get(subghz) != SubGhzRxKeyStateTX) {
            if(subghz_wardiving_txrx_hopper_get_state(subghz->txrx) != SubGhzHopperStateOFF) {
                subghz_wardiving_txrx_hopper_update(
                    subghz->txrx, subghz->last_settings->hopping_threshold);
                subghz_scene_receiver_update_statusbar(subghz);
            }

            SubGhzThresholdRssiData ret_rssi = subghz_threshold_get_rssi_data(
                subghz->threshold_rssi, subghz_wardiving_txrx_radio_device_get_rssi(subghz->txrx));
            ...
            subghz_protocol_decoder_bin_raw_data_input_rssi(
                (SubGhzProtocolDecoderBinRAW*)subghz_wardiving_txrx_get_decoder(subghz->txrx),
                ret_rssi.rssi);
        }
