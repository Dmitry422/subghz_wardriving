...
static void subghz_scene_add_to_history_callback(
    ...
    uint16_t idx = subghz_wardiving_history_get_item(subghz->history);
    ...
    if(subghz_wardiving_history_add_to_history(
           subghz->history, decoder_base, &preset, latitude, longitude)) {
        ...
        subghz_wardiving_history_get_text_item_menu(subghz->history, item_name, idx);
        subghz_wardiving_history_get_time_item_menu(subghz->history, item_time, idx);
        subghz_view_receiver_add_item_to_menu(
            subghz->subghz_receiver,
            furi_string_get_cstr(item_name),
            furi_string_get_cstr(item_time),
            subghz_wardiving_history_get_type_protocol(subghz->history, idx),
            subghz_wardiving_history_get_repeats(subghz->history, idx));
...
bool subghz_scene_decode_raw_start(SubGhz* subghz) {
    ...
    bool success = false;
    do {
        if(!flipper_format_rewind(subghz_wardiving_txrx_get_fff_data(subghz->txrx))) {
            FURI_LOG_E(TAG, "Rewind error");
            break;
        }

        if(!flipper_format_read_string(
               subghz_wardiving_txrx_get_fff_data(subghz->txrx), "File_name", file_name)) {
...
bool subghz_scene_decode_raw_next(SubGhz* subghz) {
    ...
    SubGhzReceiver* receiver = subghz_wardiving_txrx_get_receiver(subghz->txrx);
...
void subghz_scene_decode_raw_on_enter(void* context) {
    ...
    subghz_wardiving_txrx_receiver_set_filter(subghz->txrx, SubGhzProtocolFlag_Decodable);

    if(scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneDecodeRAW) ==
       SubGhzDecodeRawStateStart) {
        //Decode RAW to history
        subghz_wardiving_history_reset(subghz->history);
        ...
        for(uint16_t i = 0; i < subghz_wardiving_history_get_item(subghz->history); i++) {
            furi_string_reset(item_name);
            furi_string_reset(item_time);
            subghz_wardiving_history_get_text_item_menu(subghz->history, item_name, i);
            subghz_wardiving_history_get_time_item_menu(subghz->history, item_time, i);
            subghz_view_receiver_add_item_to_menu(
                subghz->subghz_receiver,
                furi_string_get_cstr(item_name),
                furi_string_get_cstr(item_time),
                subghz_wardiving_history_get_type_protocol(subghz->history, i),
                subghz_wardiving_history_get_repeats(subghz->history, i));
        }
