...
void subghz_scene_show_gps_draw_satellites(void* context) {
    ...
    if(scene_manager_get_scene_state(subghz->scene_manager, SubGhzSceneShowGps)) {
        latitude = subghz_wardiving_txrx_get_latitude(subghz->txrx);
        longitude = subghz_wardiving_txrx_get_longitude(subghz->txrx);
    } else {
        latitude = subghz_wardiving_history_get_latitude(subghz->history, subghz->idx_menu_chosen);
        longitude =
            subghz_wardiving_history_get_longitude(subghz->history, subghz->idx_menu_chosen);
    }
