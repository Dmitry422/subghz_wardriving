    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        if(event.event == SubGhzCustomEventSceneExit) {
            ...
            if(state == SubGhzRpcStateLoaded) {
                ...
                switch(subghz_wardiving_txrx_tx_start(
                    subghz->txrx, subghz_wardiving_txrx_get_fff_data(subghz->txrx))) {
            ...
            //     if(state == SubGhzRpcStateLoaded) {
            //         switch(
            //             subghz_wardiving_txrx_tx_start(subghz->txrx, subghz_wardiving_txrx_get_fff_data(subghz->txrx))) {
            //         case SubGhzTxRxStartTxStateErrorOnlyRx:
            //             rpc_system_app_set_error_code(
            //                 subghz->rpc_ctx, RpcAppSystemErrorCodeRegionLock);
            //             rpc_system_app_set_error_text(
