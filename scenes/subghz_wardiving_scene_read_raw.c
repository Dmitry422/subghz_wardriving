...
bool subghz_scene_read_raw_update_filename(SubGhz* subghz) {
    ...
    do {
        FlipperFormat* fff_data = subghz_wardiving_txrx_get_fff_data(subghz->txrx);
...
static void subghz_scene_read_raw_update_statusbar(void* context) {
    ...
    subghz_read_raw_set_radio_device_type(
        subghz->subghz_read_raw, subghz_wardiving_txrx_radio_device_get(subghz->txrx));
}
...
void subghz_scene_read_raw_on_enter(void* context) {
    ...
    furi_check(subghz_wardiving_txrx_load_decoder_by_name_protocol(
        subghz->txrx, SUBGHZ_PROTOCOL_RAW_NAME));

    //set filter RAW feed
    subghz_wardiving_txrx_receiver_set_filter(subghz->txrx, SubGhzProtocolFlag_RAW);
...
bool subghz_scene_read_raw_on_event(void* context, SceneManagerEvent event) {
    ...
    SubGhzProtocolDecoderRAW* decoder_raw =
        (SubGhzProtocolDecoderRAW*)subghz_wardiving_txrx_get_decoder(subghz->txrx);
    if(event.type == SceneManagerEventTypeCustom) {
            ...
            if((subghz_rx_key_state_get(subghz) == SubGhzRxKeyStateAddKey) ||
               (subghz_rx_key_state_get(subghz) == SubGhzRxKeyStateBack)) {
                ...
                if(subghz->raw_send_only) {
                    subghz_wardiving_txrx_set_default_preset(subghz->txrx, 0);
                } else {
        ...
        case SubGhzCustomEventViewReadRAWSendStart:
            ...
            if(subghz_file_available(subghz) && subghz_scene_read_raw_update_filename(subghz)) {
                //start send
                subghz->state_notifications = SubGhzNotificationStateIDLE;
                if(!subghz_tx_start(subghz, subghz_wardiving_txrx_get_fff_data(subghz->txrx))) {
                    ...
                    // set callback end tx
                    subghz_wardiving_txrx_set_raw_file_encoder_worker_callback_end(
