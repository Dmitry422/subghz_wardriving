...
#include "subghz_wardiving_scene.h"
#include <lib/subghz/blocks/custom_btn.h>

#include "../helpers/subghz_wardiving_txrx_i.h"
...
static bool subghz_scene_receiver_info_update_parser(void* context) {
    ...
    if(subghz_wardiving_txrx_load_decoder_by_name_protocol(
           subghz->txrx,
           subghz_wardiving_history_get_protocol_name(subghz->history, subghz->idx_menu_chosen))) {
        ...
        subghz_protocol_decoder_base_deserialize(
            ...
            subghz_wardiving_history_get_raw_data(subghz->history, subghz->idx_menu_chosen));

        SubGhzRadioPreset* preset =
            subghz_wardiving_history_get_radio_preset(subghz->history, subghz->idx_menu_chosen);
...
void subghz_scene_receiver_info_draw_widget(SubGhz* subghz) {
    if(subghz_scene_receiver_info_update_parser(subghz)) {
        ...
        subghz_protocol_decoder_base_get_string(
            subghz_wardiving_txrx_get_decoder(subghz->txrx), text);
        ...
        if(subghz_wardiving_txrx_protocol_is_serializable(subghz->txrx)) {
            widget_add_button_element(
                subghz->widget,
                GuiButtonTypeRight,
                "Save",
        ...
        if(subghz_wardiving_txrx_protocol_is_transmittable(subghz->txrx, false)) {
            widget_add_button_element(
                subghz->widget,
                GuiButtonTypeCenter,
                "Send",
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == SubGhzCustomEventSceneReceiverInfoTxStart) {
            ...
            if(subghz_wardiving_txrx_protocol_is_serializable(subghz->txrx)) {
                ...
                subghz->save_datetime = subghz_wardiving_history_get_datetime(
                    subghz->history, subghz->idx_menu_chosen);
...
void subghz_scene_receiver_info_on_exit(void* context) {
    ...
    widget_reset(subghz->widget);
    subghz_wardiving_txrx_reset_dynamic_and_custom_btns(subghz->txrx);
}
